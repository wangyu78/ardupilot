...
class Copter : public AP_Vehicle {
   ...
   private:
    ...
    struct RangeFinderState {
        ...
        int8_t
            glitch_count;  // non-zero number indicates rangefinder is glitching
    ...
    class SurfaceTracking {
       public:
        ...
        enum class Surface { NONE = 0, GROUND = 1, CEILING = 2 };
       ...
       private:
        ...
        bool reset_target;  // true if target should be reset because of change
                            // in tracking_state
    } surface_tracking;
...
#include USERHOOK_VARIABLES
    ...
    typedef union {
        struct {
            uint8_t unused1 : 1;      // 0
            uint8_t simple_mode : 2;  // 1,2     // This is the state of simple
                                      // mode : 0 = disabled ; 1 = SIMPLE ; 2 =
                                      // SUPERSIMPLE
            uint8_t pre_arm_rc_check : 1;  // 3       // true if rc input
                                           // pre-arm checks have been completed
                                           // successfully
            uint8_t pre_arm_check : 1;  // 4       // true if all pre-arm checks
                                        // (rc, accel calibration, gps lock)
                                        // have been performed
            uint8_t auto_armed : 1;     // 5       // stops auto missions from
                                        // beginning until throttle is raised
            uint8_t
                logging_started : 1;  // 6       // true if logging has started
            uint8_t land_complete : 1;  // 7       // true if we have detected a
                                        // landing
            uint8_t new_radio_frame : 1;  // 8       // Set true if we have new
                                          // PWM data to act on from the Radio
            uint8_t usb_connected_unused : 1;  // 9       // UNUSED
            uint8_t rc_receiver_present : 1;   // 10      // true if we have an
                                               // rc receiver present (i.e. if
                                               // we've ever received an update
            uint8_t compass_mot : 1;  // 11      // true if we are currently
                                      // performing compassmot calibration
            uint8_t motor_test : 1;   // 12      // true if we are currently
                                      // performing the motors test
            uint8_t
                initialised : 1;  // 13      // true once the init_ardupilot
                                  // function has completed.  Extended status to
                                  // GCS is not sent until this completes
            uint8_t land_complete_maybe : 1;  // 14      // true if we may have
                                              // landed (less strict version of
                                              // land_complete)
            uint8_t throttle_zero : 1;  // 15      // true if the throttle stick
                                        // is at zero, debounced, determines if
                                        // pilot intends shut-down when not
                                        // using motor interlock
            uint8_t
                system_time_set_unused : 1;  // 16      // true if the system
                                             // time has been set from the GPS
            uint8_t gps_glitching : 1;    // 17      // true if GPS glitching is
                                          // affecting navigation accuracy
            uint8_t using_interlock : 1;  // 20      // aux switch motor
                                          // interlock function is in use
            uint8_t land_repo_active : 1;  // 21      // true if the pilot is
                                           // overriding the landing position
            uint8_t motor_interlock_switch : 1;  // 22      // true if pilot is
                                                 // requesting motor interlock
                                                 // enable
            uint8_t in_arming_delay : 1;  // 23      // true while we are armed
                                          // but waiting to spin motors
            uint8_t
                initialised_params : 1;  // 24      // true when the all
                                         // parameters have been initialised. we
                                         // cannot send parameters to the GCS
                                         // until this is done
            uint8_t compass_init_location : 1;  // 25      // true when the
                                                // compass's initial location
                                                // has been set
            uint8_t
                unused2 : 1;  // 26      // aux switch rc_override is allowed
    ...
    // Failsafe
    struct {
        uint32_t last_heartbeat_ms;  // the time when the last HEARTBEAT message
                                     // arrived from a GCS - used for triggering
                                     // gcs failsafe
        uint32_t terrain_first_failure_ms;  // the first time terrain data
                                            // access failed - used to calculate
        ...
        uint8_t terrain : 1;  // true if the missing terrain data failsafe has
                              // occurred
    ...
    // is reset at each arming or in SuperSimple mode when the vehicle leaves a
    ...
    AP_BattMonitor battery{
        MASK_LOG_CURRENT,
        FUNCTOR_BIND_MEMBER(&Copter::handle_battery_failsafe, void,
                            const char *, const int8_t),
    ...
    // the 4th element, so it can reject 3 low glitches, and 1 high glitch. This
    // is because any "off" glitches can be highly problematic for a helicopter
    // running an ESC governor.  Even a single "off" frame can cause the rotor
    // to slow dramatically and take a long time to restart.
    ModeFilterInt16_Size5 rotor_speed_deglitch_filter{4};

    // Tradheli flags
    typedef struct {
        uint8_t dynamic_flight : 1;   // 0   // true if we are moving at a
                                      // significant speed (used to turn on/off
                                      // leaky I terms)
        uint8_t inverted_flight : 1;  // 1   // true for inverted flight mode
        uint8_t
            in_autorotation : 1;  // 2   // true when heli is in autorotation
    } heli_flags_t;
...
#define FAILSAFE_LAND_PRIORITY 1
    static_assert(
        _failsafe_priorities[FAILSAFE_LAND_PRIORITY] == Failsafe_Action_Land,
    ...
    void Log_Write_Parameter_Tuning(uint8_t param, float tuning_val,
                                    float tune_min, float tune_max);
    ...
    void Log_Write_SysID_Setup(uint8_t systemID_axis, float waveform_magnitude,
                               float frequency_start, float frequency_stop,
                               float time_fade_in, float time_const_freq,
                               float time_record, float time_fade_out);
    void Log_Write_SysID_Data(float waveform_time, float waveform_sample,
                              float waveform_freq, float angle_x, float angle_y,
                              float angle_z, float accel_x, float accel_y,
                              float accel_z);
