...
// optional force parameter used to force the flight mode change (used only
// first time mode is set) returns true if mode was successfully set ACRO,
// STABILIZE, ALTHOLD, LAND, DRIFT and SPORT can always be set successfully but
// the return state of other flight modes should be checked and the caller
// should deal with failures appropriately
bool Copter::set_mode(Mode::Number mode, ModeReason reason) {
    ...
    bool ignore_checks =
        !motors->armed();  // allow switching to any mode if disarmed.  We rely
                           // on the arming check to perform

#if FRAME_CONFIG == HELI_FRAME
    // do not allow helis to enter a non-manual throttle mode if the
    // rotor runup is not complete
    if (!ignore_checks && !new_flightmode->has_manual_throttle() &&
        (motors->get_spool_state() == AP_Motors::SpoolState::SPOOLING_UP ||
         motors->get_spool_state() == AP_Motors::SpoolState::SPOOLING_DOWN)) {
#if MODE_AUTOROTATE_ENABLED == ENABLED
        // if the mode being exited is the autorotation mode allow mode change
        // despite rotor not being at full speed.  This will reduce altitude loss
        // on bail-outs back to non-manual throttle modes
        bool in_autorotation_check = (flightmode != &mode_autorotate ||
                                      new_flightmode != &mode_autorotate);
#else
...
#if MODE_AUTOROTATE_ENABLED == ENABLED
    // If changing to autorotate flight mode from a non-manual throttle mode,
    // store the previous flight mode to exit back to it when interlock is
    // re-engaged
...
#endif
    if (!ignore_checks && ap.land_complete && user_throttle &&
        !copter.flightmode->has_manual_throttle() &&
        new_flightmode->get_pilot_desired_throttle() >
    ...
    if (!ignore_checks && new_flightmode->requires_GPS() &&
        !copter.position_ok()) {
        gcs().send_text(MAV_SEVERITY_WARNING,
...
#if ADSB_ENABLED == ENABLED
    adsb.set_is_auto_mode((mode == Mode::Number::AUTO) ||
                          (mode == Mode::Number::RTL) ||
                          (mode == Mode::Number::GUIDED));
#endif

#if AC_FENCE == ENABLED
    // pilot requested flight mode change during a fence breach indicates pilot
    // is attempting to manually recover this flight mode change could be
    // automatic (i.e. fence, battery, GPS or GCS failsafe) but it should be
    // harmless to disable the fence temporarily in these situations as well
...
#endif
    return copter.set_mode(static_cast<Mode::Number>(new_mode),
                           ModeReason::GCS_COMMAND);
}

// update_flight_mode - calls the appropriate attitude controllers based on
// flight mode called at 100hz or more
void Copter::update_flight_mode() {
    surface_tracking
        .invalidate_for_logging();  // invalidate surface tracking alt, flight
                                    // mode will set to true if used
...
void Copter::exit_mode(Mode *&old_flightmode, Mode *&new_flightmode) {
    ...
    // smooth throttle transition when switching from manual to automatic flight
    // modes
    if (old_flightmode->has_manual_throttle() &&
        !new_flightmode->has_manual_throttle() && motors->armed() &&
        !ap.land_complete) {
        // this assumes all manual flight modes use get_pilot_desired_throttle
        // to translate pilot input to output throttle
    ...
    // stab col ramp value should be pre-loaded to the correct value to avoid a
    // twitch heli_stab_col_ramp should really only be active switching between
    // Stabilize and Acro modes
    if (!old_flightmode->has_manual_throttle()) {
...
// notify_flight_mode - sets notify object based on current flight mode.  Only
// used for OreoLED notify device
...
// get_pilot_desired_angle - transform pilot's roll or pitch input into a
// desired lean angle returns desired angle in centi-degrees
void Mode::get_pilot_desired_lean_angles(float &roll_out, float &pitch_out,
                                         float angle_max,
    ...
    roll_out = (18000 / M_PI) * atanf(cosf(pitch_out * (M_PI / 18000)) *
                                      tanf(roll_out * (M_PI / 18000)));
...
bool Mode::_TakeOff::triggered(const float target_climb_rate) const {
    ...
    if (copter.motors->get_spool_state() !=
...
void Mode::zero_throttle_and_relax_ac(bool spool_up) {
    ...
    attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(0.0f, 0.0f,
                                                                  0.0f);
...
void Mode::make_safe_spool_down() {
    ...
    switch (motors->get_spool_state()) {
        ...
        case AP_Motors::SpoolState::SPOOLING_DOWN:
            // while transitioning though active states continue to operate
            // normally
            break;
    }

    pos_control->relax_alt_hold_controllers(
        0.0f);  // forces throttle output to go to zero
...
int32_t Mode::get_alt_above_ground_cm(void) {
    ...
    if (copter.rangefinder_alt_ok()) {
        ...
        if (!navigating ||
            !copter.current_loc.get_alt_cm(Location::AltFrame::ABOVE_TERRAIN,
                                           alt_above_ground)) {
...
void Mode::land_run_vertical_control(bool pause_descent) {
    ...
    bool doing_precision_landing = !copter.ap.land_repo_active &&
    ...
    if (!pause_descent) {
        ...
        max_land_descent_velocity =
            MIN(max_land_descent_velocity, -abs(g.land_speed));

        // Compute a vertical velocity demand such that the vehicle approaches
        // g2.land_alt_low. Without the below constraint, this would cause the
        // vehicle to hover at g2.land_alt_low.
        cmb_rate = AC_AttitudeControl::sqrt_controller(
            MAX(g2.land_alt_low, 100) - get_alt_above_ground_cm(),
            pos_control->get_pos_z_p().kP(), pos_control->get_max_accel_z(),
            G_Dt);

        // Constrain the demanded vertical velocity so that it is between the
        // configured maximum descent speed and the configured minimum descent
        // speed.
        cmb_rate = constrain_float(cmb_rate, max_land_descent_velocity,
                                   -abs(g.land_speed));
        ...
        if (doing_precision_landing && copter.rangefinder_alt_ok() &&
            ...
            float land_slowdown =
                MAX(0.0f, pos_control->get_horizontal_error() *
                              (max_descent_speed / precland_acceptable_error));
            cmb_rate = MIN(-precland_min_descent_speed,
                           -max_descent_speed + land_slowdown);
        }
    }
...
void Mode::land_run_horizontal_control() {
    ...
    if (!copter.failsafe.radio) {
        ...
        if (g.land_repositioning) {
            ...
            get_pilot_desired_lean_angles(
                target_roll, target_pitch, loiter_nav->get_angle_max_cd(),
    ...
    if (g2.wp_navalt_min > 0) {
        ...
        float attitude_limit_cd = linear_interpolate(
            700, copter.aparm.angle_max, get_alt_above_ground_cm(),
    ...
    if (auto_yaw.mode() == AUTO_YAW_HOLD) {
        ...
        attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(
            nav_roll, nav_pitch, target_yaw_rate);
    } else {
        // roll, pitch from waypoint controller, yaw heading from auto_heading()
        attitude_control->input_euler_angle_roll_pitch_yaw(
            nav_roll, nav_pitch, auto_yaw.yaw(), true);
    }
}

float Mode::throttle_hover() const { return motors->get_throttle_hover(); }
...
float Mode::get_pilot_desired_throttle() const {
    ...
    if (throttle_control < mid_stick) {
        ...
        throttle_in = 0.5f + ((float)(throttle_control - mid_stick)) * 0.5f /
                                 (float)(1000 - mid_stick);
    }

    const float expo = constrain_float(-(thr_mid - 0.5f) / 0.375f, -0.5f, 1.0f);
    // calculate the output throttle using the given expo function
    float throttle_out = throttle_in * (1.0f - expo) +
                         expo * throttle_in * throttle_in * throttle_in;
    return throttle_out;
}
...
Mode::AltHoldModeState Mode::get_alt_hold_state(float target_climb_rate_cms) {
    ...
    if (!motors->armed()) {
        ...
        // the aircraft is currently landed or taking off, asking for a positive
        // climb rate and in THROTTLE_UNLIMITED the aircraft should progress
        // through the take off procedure
...
void Mode::update_simple_mode(void) { copter.update_simple_mode(); }

bool Mode::set_mode(Mode::Number mode, ModeReason reason) {
    return copter.set_mode(mode, reason);
}

void Mode::set_land_complete(bool b) { return copter.set_land_complete(b); }

GCS_Copter &Mode::gcs() { return copter.gcs(); }

void Mode::Log_Write_Event(Log_Event id) {
    return copter.logger.Write_Event(id);
}

void Mode::set_throttle_takeoff() { return copter.set_throttle_takeoff(); }

float Mode::get_avoidance_adjusted_climbrate(float target_rate) {
    return copter.get_avoidance_adjusted_climbrate(target_rate);
}

uint16_t Mode::get_pilot_speed_dn() { return copter.get_pilot_speed_dn(); }
